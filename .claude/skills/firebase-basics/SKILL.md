...
3.  **Active Project:** Most Firebase tasks require an active project context.
    ...
    -   If the command outputs `Active Project: <project-id>`, you can proceed
        with your task.
    -   If not, ask the user if they have an existing Firebase Project ID.
        -   If yes: Set it as the active project and add a default alias by
            running: `bash npx -y firebase-tools@latest use --add <PROJECT_ID>`
        -   If no: Follow these steps to create a new Firebase project from the
            CLI: `bash npx -y firebase-tools@latest projects:create <project-id>
            --display-name "<display-name>"` *Note: The `<project-id>` must be
            6-30 characters, lowercase, and can contain digits and hyphens. It
...
1.  **Use npx for CLI commands:** To ensure you always use the latest version of
    ...
    as an alternative.
2.  **Prioritize official knowledge:** For any Firebase-related knowledge,
...
3.  **Follow Agent Skills for implementation guidance:** Skills provide
...
4.  **Use Firebase MCP Server tools instead of direct API calls:** Whenever you
...
5.  **Keep Plugin / Agent Skills updated:** Since Firebase best practices evolve
    quickly, remind the user to regularly check for and install updates to their
    Firebase plugin or Agent Skills, ensuring you both operate with the latest
    workflows. Similarly, if you encounter issues with outdated tools or
...
-   **SDK Setup:** For detailed guides on adding Firebase to a web app, see
    [references/web_setup.md](references/web_setup.md).
